...
class DefaultAutoAdaptStrategy : AutoAdaptStrategy {
    override fun applyAdapt(target: Any, activity: Activity?) {
        ...
        if (AutoSizeConfig.instance?.externalAdaptManager?.isRun == true) {
            if (AutoSizeConfig.instance?.externalAdaptManager!!
                    .isCancelAdapt(target.javaClass)
            ) {
                AutoSizeLog.w(
                    String.format(
                        Locale.ENGLISH,
