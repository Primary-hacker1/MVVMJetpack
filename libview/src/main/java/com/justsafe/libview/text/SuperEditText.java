...
public class SuperEditText extends AppCompatEditText {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // 原理：通过 反射机制 动态设置光标
        // 1. 获取资源ID
        cursor = typedArray.getResourceId(R.styleable.SuperEditText_cursor, R.drawable.super_edittext_cursor);
        try {
        ...
        if (ed_bg) {
            setBackground(getResources().getDrawable(R.drawable.super_edittext_bg));
            /**
             * 初始化删除图标
             */
            // 1. 获取资源ID
